...
export class ResponseHandler {
    ...
    private static async handleErrorResponse<T>(response: Response, shouldThrowOnError: boolean): Promise<ApiResponse<T>> {
    ...
    private static async extractErrorContent(response: Response): Promise<{ message: string; youtubeAuthRequired: boolean }> {
    ...
    private static async extractJsonError(response: Response): Promise<{ message: string; youtubeAuthRequired: boolean }> {
        try {
    ...
    private static async extractTextError(response: Response): Promise<{ message: string; youtubeAuthRequired: boolean }> {
        const errorText = await response.text();
        return {
            message: errorText || 'An error occurred while fetching the data.',
            youtubeAuthRequired: false
