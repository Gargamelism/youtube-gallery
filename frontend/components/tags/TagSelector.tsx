...
export function TagSelector({ channelId, selectedTags, onTagsChange, onCreateTag }: TagSelectorProps) {
  ...
  const filteredTags = allTags.filter(
    tag => tag.name.toLowerCase().includes(searchTerm.toLowerCase()) && !selectedTagIds.includes(tag.id)
  );

  const trimmedSearchTerm = searchTerm.trim();
  const exactMatch = allTags.find(tag => tag.name.toLowerCase() === trimmedSearchTerm.toLowerCase());
    ...
    <div className="TagSelector" ref={dropdownRef}>
      <div className="TagSelector__selected-tags flex flex-wrap gap-2 mb-2">
        {selectedTags.map(tag => (
          <TagBadge key={tag.id} tag={tag} size="sm" removable onRemove={() => handleTagRemove(tag.id)} />
        ))}
      </div>
            ...
            {filteredTags.length === 0 && !onCreateTag ? (
              ...
              filteredTags.map(tag => (
                ...
                >
                  ...
                  {tag.description && <span className="text-xs text-gray-400 truncate">{tag.description}</span>}
                </button>
              ))
            )}
          </div>
