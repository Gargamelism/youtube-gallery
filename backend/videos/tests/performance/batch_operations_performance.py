...
class BatchOperationPerformanceTests(TransactionTestCase):
    ...
    def test_channel_batch_update_performance(self):
        ...
        for result in results:
            ...
            self.assertLess(
                ...
                20,
                f"Too many queries per item at scale {result['scale_factor']}: "
                f"{result['queries_per_item']:.2f}"
            )
            ...
            self.assertLess(
                result['ms_per_item'],
                200,
                f"Too slow per item at scale {result['scale_factor']}: "
                f"{result['ms_per_item']:.2f}ms"
            )
    ...
    def test_orphaned_channel_detection_performance(self):
        ...
        def detect_orphaned_operation(data: Dict[str, Any]) -> int:
            ...
            orphaned = Channel.objects.filter(
                user_subscriptions__isnull=True
            ).distinct()
        ...
        for result in results:
            self.assertLess(
                result['query_count'],
                5,
                f"Orphaned detection should use minimal queries, got {result['query_count']}"
            )
            ...
            for i in range(scale):
                ...
                for j in range(3):
                    Video.objects.create(
                        channel=channel,
                        video_id=f'removal_video_{i}_{j}',
                        title=f'Video {j} from channel {i}'
                    )
                channels.append(channel)
            return channels
    ...
    def test_youtube_api_batch_calls_performance(self):
        ...
        for result in results:
            ...
            self.assertGreater(
                result['elapsed_time_ms'],
                expected_min_time,
                f"API simulation timing seems incorrect"
            )
        ...
        try:
            ...
            with DatabaseQueryCounter() as counter:
                ...
                channels_with_users = Channel.objects.filter(
                    user_subscriptions__isnull=False
                ).prefetch_related(
                    'user_subscriptions__user',
                    'videos'
                ).distinct()
            ...
            self.assertLess(
                counter.query_count,
                10,
                f"Potential N+1 query issue: {counter.query_count} queries"
            )
