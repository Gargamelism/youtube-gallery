...
class ChannelUpdatingFixtures:
    ...
    def cleanup(self):
        ...
        for object_type in ['user_channel_tags', 'channel_tags', 'user_videos',
                           'user_channels', 'videos', 'channels', 'users']:
    ...
    def create_channel_tags_and_assignments(self, users: dict, subscriptions: dict) -> dict:
        ...
        active_user_tags = [
            ChannelTag.objects.create(
                ...
                name='Tech',
                color='#3B82F6',
                description='Technology related channels'
            ),
            ChannelTag.objects.create(
                user=users['active_user'],
                name='Education',
        ...
        power_user_tags = [
            ...
            ChannelTag.objects.create(
                ...
                name='Archive',
                color='#6B7280',
                description='Old or inactive channels'
            )
        ]
            ...
            if 'tech' in subscription.channel.title.lower():
                tag_assignments.append(
                    UserChannelTag.objects.create(
                        ...
                        tag=active_user_tags[0]  # Tech tag
                    )
                )
                ...
                tag_assignments.append(
                    UserChannelTag.objects.create(
                        user_channel=subscription,
                        tag=active_user_tags[1]  # Education tag
                    )
                )
            else:
                tag_assignments.append(
                    UserChannelTag.objects.create(
                        user_channel=subscription,
                        tag=active_user_tags[2]  # Entertainment tag
                    )
                )
        ...
        for subscription in subscriptions['power_user_subs']:
            if subscription.channel.title in ['Active Tech Channel', 'Popular Education Channel']:
                tag_assignments.append(
                    UserChannelTag.objects.create(
                        user_channel=subscription,
                        tag=power_user_tags[0]  # High Priority tag
                    )
                )
            else:
                tag_assignments.append(
                    UserChannelTag.objects.create(
                        user_channel=subscription,
                        tag=power_user_tags[1]  # Archive tag
                    )
                )
    ...
    def create_complete_test_scenario(self) -> dict:
        ...
        scenario['subscriptions'] = self.create_user_subscriptions(
            scenario['users'],
            scenario['channels']
        )
        scenario['tags'] = self.create_channel_tags_and_assignments(
            scenario['users'],
            scenario['subscriptions']
        )
        scenario['interactions'] = self.create_user_video_interactions(
            scenario['users'],
            scenario['videos']
        )
