...
class YouTubeImportQuotaIntegrationTests(TestCase):
    ...
    def test_get_channel_by_id_checks_quota(self):
        """Test that _get_channel_by_id checks quota before making API call"""
        with patch('videos.services.youtube.build') as mock_build:
            mock_youtube = Mock()
            ...
            with self.assertRaises(Exception) as context:
                youtube_service._get_channel_by_id("UC123456")
    ...
    def test_get_channel_by_id_records_quota_usage(self):
        """Test that _get_channel_by_id records quota usage after successful API call"""
        with patch('videos.services.youtube.build') as mock_build:
            mock_youtube = Mock()
            mock_build.return_value = mock_youtube
            ...
            result = youtube_service._get_channel_by_id("UC123456")
    ...
    def test_get_channel_videos_records_quota_usage(self):
        ...
        with patch('videos.services.youtube.build') as mock_build:
            ...
            expected_calls = [
                Mock.call("playlistItems.list"),
                Mock.call("videos.list")
            ]
